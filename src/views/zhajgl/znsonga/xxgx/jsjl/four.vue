      ...
      >
        ...
        <el-table-column label="操作">
          <template slot-scope="scope">
            ...
            <el-button type="text" size="small" @click="handleBack(scope.row)">接收</el-button>
          </template>
        </el-table-column>
      </el-table>
      <!-- 分页start-->
  ...
  data() {
    return {
      ...
      tableHeader: [
        ...
        { filed: 'handleTime', title: '接访时间' },
        { filed: 'visitorName', title: '信访人姓名' },
        { filed: 'typeOptionName', title: '信访类型' },
  ...
  methods: {
    ...
    handleBack(row) {
      this.$confirm('你是否确认接收选中的档案?', '提示', {
        ...
        this.$api.jieshouhuajie(row.id).then(res => {
        console.log(res)
        if (res.code === 0) {
